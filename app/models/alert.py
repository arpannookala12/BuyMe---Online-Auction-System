...
class Alert(db.Model):
    ...
    def matches_auction(self, auction):
        ...
        # Category check
        if self.category_id and auction.item.category_id != self.category_id:
            parent_cat = auction.item.category.parent_id
            # Check parent categories too
            while parent_cat:
                if parent_cat == self.category_id:
                    break
        ...
        if self.keywords:
            keywords = [k.strip().lower() for k in self.keywords.split(',')]
            item_text = (auction.item.name + ' ' + (auction.item.description or '')).lower()
            if not any(k in item_text for k in keywords):
                return False
